...
def get_args():
    ...
    parser.add_argument('--pt_pred_path', type=str, help='3d segmentation output')
...
if __name__ == "__main__":
    ...
    pt_pred = np.load("/mnt/luoly/SAMPro3D-main/dataset/530_scannet_table0/scene0000_01/sampro3d_predictions/scene0000_01_seg_floor.npy")
    ...
    for i in range(N):
        ...
        if label not in groups:
            groups[label] = []
        groups[label].append(position.reshape(1, 3))
    groups_counts = []
    for label, group in groups.items():
        count = len(group)
        groups_counts.append((label, count))
    groups_counts.sort(key=lambda x: x[1], reverse=True)
    c_all = []
    xyz_all = []
    for label, count in groups_counts:
        c = cmap[label]
        c = [x for x in c]
        color = np.array([c[0], c[1], c[2]]).reshape(1, 3)
    
        colors = np.tile(color, (count, 1, 1))
        labels = np.tile(label, count)
        # c_all.append(colors)
        # xyz_all.append(np.array(groups[label]))
        xyz = np.concatenate(np.array(groups[label]))
    ...
    # c = np.concatenate(c_all)
        ...
        output_vis_file = os.path.join(args.output_vis_path, args.scene_name + "_" + str(label) +  '_seg.ply')
        print(output_vis_file)
        mesh, _ = mesh.remove_statistical_outlier(nb_neighbors=1000, std_ratio=0.3)
        o3d.io.write_point_cloud(output_vis_file, mesh)
