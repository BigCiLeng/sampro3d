...
def process_batch(
    ...
    # for point in points:
    #     ins_id = mask[int(point[1]), int(point[0])]
    #     if ins_id == 0:
    #         temp_iou = 0.0
    #         temp_mask = torch.zeros(mask.shape).to(mask.device)
    #     else:
    #         temp_iou = 1.0
    #         temp_mask = mask.clone().detach()
    #         temp_mask[~(mask == ins_id)] = 0
    #     masks.append(temp_mask.unsqueeze(0))
    #     iou_preds.append(torch.tensor(temp_iou).view(1, -1).to(mask.device))
    # masks = mask
    # masks = torch.concatenate(masks, dim=0).to(device=mask.device)
    # iou_preds = torch.concatenate(iou_preds, dim=0).to(device=mask.device)
    # Serialize predictions and store in MaskData  T
...
def generate_sam_gt_output(frame_id_init, frame_id_end, init_prompt, args):
    for i in trange(frame_id_init, frame_id_end):
        ...
        # Load the intrinsic
        depth_intrinsic = torch.tensor(np.loadtxt(os.path.join(args.data_path, 'intrinsics.txt')), dtype=torch.float64).to(device=args.device)
        ...
        pose = torch.tensor(np.loadtxt(os.path.join(args.data_path, args.scene_name, 'pose', str(frame_name) + '.txt')), dtype=torch.float64).to(device=args.device)
        mask = torch.from_numpy(np.array(Image.open(os.path.join(args.data_path, args.scene_name, 'masks', '{:05d}.png'.format(int(frame_name[6:]) - 1))))).to(device=args.device)
...
if __name__ == "__main__":
    ...
    scene_plypath = os.path.join(args.data_path, args.scene_name, args.scene_name + '_sparse.ply')
    ...
    generate_sam_gt_output(frame_id_init, frame_id_end, init_prompt, args)
